...
export async function getSavedChapter(work_id: string, chapter_number: string): Promise<ChapterResponse | undefined> {
    const database = await getDatabase();
    const transaction = database.transaction(CHAPTERS_STORE, "readonly");
    const request = transaction.objectStore(CHAPTERS_STORE).index(WORK_NUMBER_KEY).get([work_id, chapter_number]);

    return waitForRequest<ChapterResponse | undefined>(request);
}

export async function deleteSavedChapter(
    work_id: string,
    ...
    const id = await waitForRequest(
        store.index(WORK_NUMBER_KEY).getKey([work_id, chapter_number]),
    );
